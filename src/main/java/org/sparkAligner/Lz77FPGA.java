...
public class Lz77FPGA {
    ...
    public static byte[] compress(SWIGTYPE_p_unsigned_char FPGAinput, int length) {
        ...
        byte[] returnBytes = new byte[compressed_byte_num];
        for(int i = 0 ; i < compressed_byte_num ; i++){
            returnBytes[i] = (byte)compression_core.uint8_t_array_getitem(encoded_bytes, i) ;
        }
